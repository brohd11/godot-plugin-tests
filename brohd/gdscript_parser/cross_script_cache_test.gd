...
static func _run(out: Array) -> int:
	...
	fails += _check(out, "mutual-preload cycle does not loop the cache", _test_cycle_no_cache_loop())
...
# The gp_cycle fixtures preload each other (and b extends a's preload const), so parsing or caching
# one pulls in the other and the cycle closes. The assertion is twofold: this function returns at all
# (a naive dependency walk loops forever), and both sides rehydrate as CACHED_RESOLVED.
static func _test_cycle_no_cache_loop() -> String:
	var a := DIR + "fixtures/gp_cycle_a.gd"
	var b := DIR + "fixtures/gp_cycle_b.gd"
	_make_live_for(b).write_cache()
	_make_live_for(a).write_cache()
	for path in [a, b]:
		var cached := _fresh_dispatcher().read_cache(path)
		if cached == null:
			return "read_cache returned null for %s" % path.get_file()
		if cached.state != GDScriptParser.STATE_CACHED_RESOLVED:
			return "%s did not rehydrate as CACHED_RESOLVED" % path.get_file()
	# resolve across the cycle on the rehydrated parser: b's `phase` is typed a's Phase enum
	var cached_b := _fresh_dispatcher().read_cache(b)
	var tr = cached_b.get_class_object("").get_member_type_rich("phase")
	if str(tr.get("type", "")) == "":
		return "rehydrated parser did not resolve 'phase' across the cycle"
	return ""
