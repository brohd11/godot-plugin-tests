...
func _run():
	...
	await _test_editor_behavior(ctx)
	print("Editor Console: %d checks, %d failures" % [checks, failures])
...
func _check_scripts(directory:String):
	for file in DirAccess.get_files_at(directory):
		if file.ends_with(".gd"):
			...
			check(script != null and script.can_instantiate(), "parse " + directory.path_join(file))
